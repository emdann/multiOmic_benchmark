...
umap_labels_pl <- ggpubr::ggarrange(
    DimPlot(orig.RNA.seu, group.by="annotation", label = TRUE, cols=cell.type.pal) + ggtitle("scRNA-seq"),
  plotlist = list(
    DimPlot(atac.seu, reduction = "umap.snap", group.by = "predicted.id_CCA"  , cols=cell.type.pal, label=TRUE, repel=F) + ggtitle("scATAC-seq - CCA"),
    DimPlot(atac.seu, reduction = "umap.snap", group.by = "predicted.id_Liger", cols=cell.type.pal, label=TRUE, repel=F) + ggtitle("scATAC-seq - Liger"),
    DimPlot(atac.seu, reduction = "umap.snap", group.by = "predicted.id_Conos", cols=cell.type.pal, label=TRUE, repel=F) + ggtitle("scATAC-seq - Conos")
    ),
  common.legend = TRUE, ncol=2, nrow=2
  ) 

umap_labels_pl +
  ggsave(paste0(outdir, "umap_labels.png"), width=12, height = 12)
...
plot_grid(plotlist = list(predict_score_hist + theme(legend.position = "none"), NULL,predict_score_umaps, leg), nrow=2, ncol=2, rel_heights = c(1,1.2), rel_widths = c(2,0.5)) +
