...
umap_labels_pl <- ggpubr::ggarrange(
  ...
  plotlist = list(
    DimPlot(atac.seu, reduction = "umap.snap", group.by = "predicted.id_CCA"  , cols=cell.type.pal, label=TRUE, repel=T, label.size = 3) + ggtitle("scATAC-seq - CCA"),
    DimPlot(atac.seu, reduction = "umap.snap", group.by = "predicted.id_Liger", cols=cell.type.pal, label=TRUE, repel=T, label.size = 3) + ggtitle("scATAC-seq - Liger"),
    DimPlot(atac.seu, reduction = "umap.snap", group.by = "predicted.id_Conos", cols=cell.type.pal, label=TRUE, repel=T, label.size = 3) + ggtitle("scATAC-seq - Conos")
    ),
  common.legend = TRUE, ncol=2, nrow=2
  ) 

umap_labels_pl +
  ggsave(paste0(outdir, "umap_labels.png"), width=9, height = 9)
...
umap_labels_pl_long <- ggpubr::ggarrange(
  plotlist = list(
    ...
    DimPlot(atac.seu, reduction = "umap.snap", group.by = "predicted.id_Conos", cols=cell.type.pal, label=TRUE, repel=T, label.size = 3) + ggtitle("Conos") + theme(axis.title.y = element_blank(), axis.ticks=element_blank(), axis.text = element_blank()),
    DimPlot(atac.seu, reduction = "umap.snap", group.by = "predicted.id_Liger", cols=cell.type.pal, label=TRUE, repel=T, label.size = 3) + ggtitle("Liger") + theme(axis.title.y = element_blank(), axis.ticks=element_blank(), axis.text = element_blank()),
    DimPlot(orig.RNA.seu, group.by="annotation", label = TRUE, cols=cell.type.pal, label.size = 3, repel=T) + ggtitle("scRNA-seq") + theme(axis.title.y = element_blank(), axis.ticks=element_blank(), axis.text = element_blank())
    ) ,
...
uamp_labels_pl <- ggpubr::ggarrange(
  plotlist = list(
...
predict_score_umaps <- ggpubr::ggarrange(
  plotlist = list(umap.score.cca + guides(color="none"), umap.score.conos, umap.score.liger) %>%
    map(~ .x + theme(axis.title.y = element_blank(), axis.ticks=element_blank(), axis.text = element_blank())),
...
fig.1 <- plot_grid(
  # umap_labels_pl,
  plotlist = list(predict_score_hist + theme(legend.position = "none"), predict_score_umaps, knn_score_pl + guides(alpha="none")),
  rel_heights = c(1, 1, 1.3), rel_widths = c(2,1), nrow=3)
...
plot_grid( fig.2, fig.1, rel_widths=c(1.5,1)) +
  ggsave(paste0(outdir, "fig_PBMC.png"), width=18, height = 7)
```
