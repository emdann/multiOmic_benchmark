...
```{r}
hvg.rna <- HVG_Seurat(f74.rna, nfeatures = 4000) %>% {.[. %in% rownames(f74.atac)]}
...
```{r}
f74.atac.seu <- as.Seurat(f74.atac)
f74.atac.seu <- ScaleData(f74.atac.seu, do.center = TRUE)
f74.atac.seu <- RunPCA(f74.atac.seu, features = hvg.rna) 
f74.atac.seu <- RunUMAP(f74.atac.seu, reduction = "pca", dims=1:30) 
...
## Clustering 
```{r}
f74.atac.seu <- FindNeighbors(f74.atac.seu)
f74.atac.seu <- FindClusters(f74.atac.seu, algorithm = 4)
...
```{r, fig.width=10, fig.height=10}
FeaturePlot(f74.atac.seu.atachvg, features = c('PTPRC','CD4','CD8A','CD8B','CD79A','FOXN1','EPCAM','PDGFRA','GNG4', 'FOXP3','RAG1','RAG2','PTCRA','IL7R','NKG7','CCR7'))
```
...
```{r}
pheatmap::pheatmap(marker.cormat.atac, main="co-accessibility")
pheatmap::pheatmap(marker.cormat.rna, main="co-expression")
```
```{r}
long.cormat.atac <- 
