...
DimPlot(f74.atac.seu, reduction = "umap", group.by = 'ident') + ggtitle("leiden") +
  ggsave("~/output/ATAC_EDA/ATAC_hvg_clustering.jpeg", height = 10, width=12)
```
<!-- ncd -->
<!-- ```{r} -->
...
```{r}
jpeg("~/output/ATAC_EDA/ATAC_hvg_coaccessibility.jpeg", height = 700, width=900)
pheatmap::pheatmap(marker.cormat.atac, main="co-accessibility", cellwidth = 10, cellheight = 10)
dev.off()
jpeg("~/output/ATAC_EDA/ATAC_hvg_coexpression.jpeg", height = 700, width=900)
pheatmap::pheatmap(marker.cormat.rna, main="co-expression", cellwidth = 10, cellheight = 10)
dev.off()
```
...
full_join(long.cormat.atac, long.cormat.rna) %>%
  ...
  ggsave("~/output/ATAC_EDA/ATAC_hvg_markersCoexpression.jpeg", height = 8, width=8)
...
annotation.df$X %>% length()

table(annotation.df$X) %>% sort(decreasing = T)
annotation.df[annotation.df$X == 'F74_1_AGCTCCTCAAGGCTCC',]

annotation.df %>% dplyr::rename(cell=X)
...
DimPlot(f74.rna.seu, dims=1:2, group.by = "annotation", reduction = "umap")
```
