...
### ATAC-seq
Methods for alignment with scRNA-seq data mostly require to reduce accessibility signal to gene-level features. There are a number of possible ways to do this:

- **Seurat pipeline**: raw 10x sequencing reads were preprocessed to call peaks with Cellranger (v ...). Then all counts in peaks are summed up all counts in peaks within gene bodies + 2kb upstream (). 
...
Could be done on the time series data 
