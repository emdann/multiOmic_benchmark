...
### Exploratory data analysis on scATAC-seq
...
| Method       | Reference | Description | Included in benchmark | Reason for Excluding |
| -------------|---------------|:---------------------------------|:-----:|:--------------------:|
| Seurat CCA   | @stuartComprehensiveIntegrationSingleCell2019a | Pairing of cells based on Canonical Correlation analysis | Yes | / |
| LIGER | @welchSingleCellMultiomicIntegration2019a | Joint Non-Negative Matrix factorization | Yes | / |
| Conos | Barkas et al. 2019 @barkasJointAnalysisHeterogeneous2019 | Joint neighboorhood graph construction | Yes | / |
| scGen | @lotfollahiScGenPredictsSinglecell2019 | Joint embedding with Variational Autoencoder | No | Requires cell type annotation in both datasets |
| totalVI | @gayosoJointModelRNA2019 | generative model + variational inference | No | Requires  multi-omic data from the same single-cells |
| BBKNN | @polanskiBBKNNFastBatch | Joint neighboorhood graph construction | No | Bad alignment during testing |
| Cusanovich2018 | @cusanovichSingleCellAtlasVivo2018a | Joint neighboorhood graph construction | No | Code unavailable |
