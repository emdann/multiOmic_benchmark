---
title: "Label transfer EDA - Thymus count gmat - union HVG"
output:
  html_document:
    df_print: paged
---
...
outdir <- "~/multiOmic_benchmark/report/output/20191113_labelTransferEDA_F74_v2_countsgmat/"
ifelse(!dir.exists(outdir), dir.create(outdir), FALSE)
```


```{r}
seu.cca <- readRDS("~/models2/labelTransferCCA_F74_countmat_unionHVGnHVGFeatures.RDS")
seu.liger <- readRDS("~/models2/labelTransferLiger_F74_countmat_unionHVGnHVGFeatures.RDS")
seu.conos <- readRDS("~/models2/labelTransferConos_F74_countmat_unionHVGnHVGFeatures.RDS")

integrate_features <- scan("~/my_data/intFeatures_unionHVGnHVG_F74_gmat.txt", what='')
...
plot_grid(
  plotlist = list(
    DimPlot(atac.seu, reduction = "UMAP") + ggtitle("scATAC-seq") + theme(axis.ticks=element_blank(), axis.text = element_blank(), legend.position="none"),
  ...
  ggsave("~/multiOmic_benchmark/report/output/F74_before.png", width=11, height = 5)
```
...
```{r, message=FALSE}
## Load original data
orig.ATAC <- readRDS("~/my_data/F74_ATAC_snapAtac_processed.RDS")
...
umap_labels_pl +
  ggsave(paste0(outdir, "umap_labels.png"), width=14, height = 5)
...
```{r}
KNN_purity_pl <- bind_rows(knn_score_df, knn_score_null_df) %>%
  group_by(method) %>%
  summarise(D=max(D)) %>%
...
KNN_purity_pl + ggsave(paste0(outdir, "KNN_purity_bars.png"), height = 5, width=5) +
  ggtitle("Count GMAT - HVG union")
```
