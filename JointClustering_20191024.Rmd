...
outdir <- "~/multiOmic_benchmark/output/JointClustering_20191024/"
...
calculateAgreement <- function(int.seu, k=20){
  ...
  common_nns <- map_dbl(names(single.dataset.nn), ~ sum(nn.list.int[[.x]] %in% single.dataset.nn[[.x]])) %>%
    set_names(names(single.dataset.nn))
  agreement.score <- {sum(common_nns)/(k*length(single.dataset.nn))}
...
liger.agr.k20 <- calculateAgreement(int.seu = int.seu.list$liger, k=20)
cca.agr.k20 <- calculateAgreement(int.seu = int.seu.list$cca, k=20)
...
pl1 <- data.frame(sharedNNs_per_cell = liger.agr.k20[[1]], cell=names(liger.agr.k20[[1]])) %>%
  ...
  ggplot(aes(sharedNNs_per_cell, fill=tech)) + geom_histogram() +
  facet_wrap(tech~.) +
    xlim(0,20) +
...
DimPlot(int.liger.seu, reduction = "umap", group.by="annotation", cols = cell.type.pal)
DimPlot(int.cca.seu, reduction = "umap", group.by="annotation")
```
...
imap(int.seu.list, ~ .x@meta.data %>%
       ...
       column_to_rownames("cell") %>%
        ggplot(aes(fill=annotation, x=as.factor(seurat_clusters))) +
...
clusterPurity(int.seu.list$cca@meta.data) %>%
  arrange(-frac) %>%
...
clusterPurity(int.seu.list$liger) %>%
  arrange(-frac) %>%
  mutate(seurat_clusters=factor(seurat_clusters, levels=unique(seurat_clusters))) %>%
  ggplot(aes(seurat_clusters, frac, fill=cluster.cell.type)) +
  geom_col() +
