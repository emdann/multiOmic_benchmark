...
VariableFeaturePlot(tcells.seu, assay = "ACTIVITY")
```

```{r}
tcells.seu <- ScaleData(tcells.seu) %>% 
  RunLSI(n=30, reduction.name="lsi.activity") %>% 
  RunUMAP(dims=1:30, reduction="lsi.activity", reduction.name="umap.activity")
...
pl1 <- DimPlot(tcells.bimod.seu, reduction = "umap", group.by="seurat_clusters", label = T) + ggtitle("gmat binary")
pl2 <- DimPlot(tcells.seu, reduction = "umap.activity", group.by="seurat_clusters", label = T) + ggtitle("gmat counts")
pl3 <- DimPlot(tcells.seu, reduction = "UMAP", group.by="seurat_clusters", label=T) + ggtitle("bmat")

CombinePlots(list(pl2, pl1, pl3), legend = "top")
```
