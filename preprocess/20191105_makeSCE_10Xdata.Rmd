...
atac.sce <- as.SingleCellExperiment(bmat.seu, assay = "ACTIVITY")
SingleCellExperiment::logcounts(atac.sce) <- NULL

counts <- SingleCellExperiment::counts(atac.sce)
cpm <- apply(counts, 2, function(x) (x/sum(x))*1e+06)
cpm(atac.sce) <- as(object = cpm, Class = "dgCMatrix")

logcounts(atac.sce) <- log1p(cpm(atac.sce))
rownames(atac.sce) %<>% str_replace_all("_", "-")

saveRDS(atac.sce, "~/my_data/PBMC_ATAC_snapAtac_processed.RDS")
```
...
bim.atac.sce <- makeBinGmat(atac.sce)

saveRDS(bim.atac.sce, "~/my_data/PBMC_ATAC_snapAtac_processed_bgmat.RDS")
```
